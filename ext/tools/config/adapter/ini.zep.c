...
/**
 * Reads ini files and converts them to Tools\Config objects.
 *
 ...
 *
 * ```php
 * $config = new \Tools\Config\Adapter\Ini(
 *     "path/config-with-constants.ini",
 *     INI_SCANNER_NORMAL
 * );
 * ```
 */
ZEPHIR_INIT_CLASS(Tools_Config_Adapter_Ini) {

	ZEPHIR_REGISTER_CLASS_EX(Tools\\Config\\Adapter, Ini, tools, config_adapter_ini, tools_config_ce, tools_config_adapter_ini_method_entry, 0);
 ...
 * Ini constructor.
 */
PHP_METHOD(Tools_Config_Adapter_Ini, __construct) {
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&iniConfig))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "tools/Config/Adapter/Ini.zep", 68);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&config);
	array_init(&config);
	zephir_is_iterable(&iniConfig, 0, "tools/Config/Adapter/Ini.zep", 95);
	if (Z_TYPE_P(&iniConfig) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&iniConfig), _5, _6, _3)
		{
			...
			if (Z_TYPE_P(&directives) == IS_ARRAY) {
				...
				array_init(&sections);
				zephir_is_iterable(&directives, 0, "tools/Config/Adapter/Ini.zep", 84);
				if (Z_TYPE_P(&directives) == IS_ARRAY) {
					ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&directives), _9$$6, _10$$6, _7$$6)
					{
						...
						zephir_check_call_status();
						zephir_array_append(&sections, &_11$$7, PH_SEPARATE, "tools/Config/Adapter/Ini.zep", 81);
					...
					while (1) {
						...
						zephir_check_call_status();
							...
							zephir_check_call_status();
							zephir_array_append(&sections, &_14$$8, PH_SEPARATE, "tools/Config/Adapter/Ini.zep", 81);
				...
				if (Z_TYPE_P(&directives) == IS_ARRAY) {
					...
					if (Z_TYPE_P(&directives) == IS_ARRAY) {
						ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&directives), _22$$12, _23$$12, _20$$12)
						{
							...
							zephir_check_call_status();
							zephir_array_append(&sections, &_24$$13, PH_SEPARATE, "tools/Config/Adapter/Ini.zep", 81);
						} ZEND_HASH_FOREACH_END();
					} else {
						ZEPHIR_CALL_METHOD(NULL, &directives, "rewind", NULL, 0);
						zephir_check_call_status();
						while (1) {
							...
							zephir_check_call_status();
								...
								zephir_check_call_status();
								zephir_array_append(&sections, &_26$$14, PH_SEPARATE, "tools/Config/Adapter/Ini.zep", 81);
							ZEPHIR_CALL_METHOD(NULL, &directives, "next", NULL, 0);
							zephir_check_call_status();
						}
					}
	...
	ZEPHIR_INIT_NVAR(&directives);
	ZEPHIR_INIT_NVAR(&section);
	ZEPHIR_CALL_PARENT(NULL, tools_config_adapter_ini_ce, getThis(), "__construct", &_31, 0, &config);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
 ...
 * implementation.
 */
PHP_METHOD(Tools_Config_Adapter_Ini, cast) {
	...
	if (Z_TYPE_P(ini) == IS_ARRAY) {
		zephir_is_iterable(ini, 1, "tools/Config/Adapter/Ini.zep", 111);
		if (Z_TYPE_P(ini) == IS_ARRAY) {
 ...
 */
PHP_METHOD(Tools_Config_Adapter_Ini, parseIniString) {
