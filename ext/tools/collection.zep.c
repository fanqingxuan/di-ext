 ...
 */
/**
 * `Tools\Collection` is a supercharged object oriented array. It implements:
...
ZEPHIR_INIT_CLASS(Tools_Collection) {
	...
	ZEPHIR_REGISTER_CLASS(Tools, Collection, tools, collection, tools_collection_method_entry, 0);

	/**
	 * @var array
	 */
	zend_declare_property_null(tools_collection_ce, SL("data"), ZEND_ACC_PROTECTED);

	/**
	 * @var bool
	 */
	zend_declare_property_bool(tools_collection_ce, SL("insensitive"), 1, ZEND_ACC_PROTECTED);

	/**
	 * @var array
	 */
	zend_declare_property_null(tools_collection_ce, SL("lowerKeys"), ZEND_ACC_PROTECTED);

	tools_collection_ce->create_object = zephir_init_properties_Tools_Collection;

	zend_class_implements(tools_collection_ce, 1, zend_ce_arrayaccess);
	zend_class_implements(tools_collection_ce, 1, spl_ce_Countable);
	zend_class_implements(tools_collection_ce, 1, zend_ce_aggregate);
	zend_class_implements(tools_collection_ce, 1, zephir_get_internal_ce(SL("jsonserializable")));
	zend_class_implements(tools_collection_ce, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(Tools_Collection, get) {
	...
	zephir_array_fetch(&value, &_3, &key, PH_NOISY, "tools/Collection.zep", 133);
	if (UNEXPECTED(zephir_is_true(&cast))) {
		ZEPHIR_MAKE_REF(&value);
		ZEPHIR_CALL_FUNCTION(NULL, "settype", NULL, 1, &value, &cast);
		ZEPHIR_UNREF(&value);
		zephir_check_call_status();
	}
	RETURN_CCTOR(&value);
...
PHP_METHOD(Tools_Collection, getValues) {
	...
	zephir_read_property(&_0, this_ptr, SL("data"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_RETURN_CALL_FUNCTION("array_values", NULL, 3, &_0);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Tools_Collection, init) {
	...
	zephir_is_iterable(&data, 0, "tools/Collection.zep", 187);
...
PHP_METHOD(Tools_Collection, jsonSerialize) {
	...
	array_init(&records);
	zephir_read_property(&_0, this_ptr, SL("data"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "tools/Collection.zep", 208);
...
PHP_METHOD(Tools_Collection, remove) {
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		...
		zephir_array_fetch(&key, &lowerKeys, &element, PH_NOISY | PH_READONLY, "tools/Collection.zep", 270);
...
PHP_METHOD(Tools_Collection, toJson) {
	...
	ZEPHIR_RETURN_CALL_CE_STATIC(tools_helper_json_ce, "encode", &_0, 5, &_1, &_2);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(Tools_Collection, unserialize) {
	...
	ZEPHIR_CALL_FUNCTION(&data, "unserialize", NULL, 6, serialized);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(NULL, this_ptr, "init", NULL, 0, &data);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
