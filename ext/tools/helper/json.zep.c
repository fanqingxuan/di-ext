...
/**
 * This file is part of the Tools.
 *
 * (c) Tools Team <team@datas.com>
 *
...
ZEPHIR_INIT_CLASS(Tools_Helper_Json) {
	...
	ZEPHIR_REGISTER_CLASS(Tools\\Helper, Json, tools, helper_json, tools_helper_json_method_entry, 0);
...
PHP_METHOD(Tools_Helper_Json, decode) {
	...
	if (UNEXPECTED(!ZEPHIR_IS_LONG_IDENTICAL(&_3, 0))) {
		...
		ZEPHIR_CALL_FUNCTION(&_5$$3, "json_last_error_msg", NULL, 24);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(&_6$$3);
		ZEPHIR_CONCAT_SV(&_6$$3, "json_decode error: ", &_5$$3);
		ZEPHIR_CALL_METHOD(NULL, &_4$$3, "__construct", NULL, 25, &_6$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_4$$3, "tools/Helper/Json.zep", 60);
		ZEPHIR_MM_RESTORE();
		return;
	}
