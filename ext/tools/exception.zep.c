...
ZEPHIR_INIT_CLASS(Tools_Exception) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Tools, Exception, tools, exception, zend_exception_get_default(TSRMLS_C), tools_exception_method_entry, 0);

	zend_class_implements(tools_exception_ce, 1, zephir_get_internal_ce(SL("throwable")));
	return SUCCESS;
