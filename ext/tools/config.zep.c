 ...
 */
/**
 * `Tools\Config` is designed to simplify the access to, and the use of,
...
ZEPHIR_INIT_CLASS(Tools_Config) {
	...
	ZEPHIR_REGISTER_CLASS_EX(Tools, Config, tools, config, tools_collection_ce, tools_config_method_entry, 0);

	/**
	 * @var string
	 */
	zend_declare_property_null(tools_config_ce, SL("pathDelimiter"), ZEND_ACC_PROTECTED);

	zephir_declare_class_constant_string(tools_config_ce, SL("DEFAULT_PATH_DELIMITER"), ".");
...
PHP_METHOD(Tools_Config, merge) {
	...
	if (_0) {
		_0 = zephir_instance_of_ev(toMerge, tools_config_ce);
	}
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&config);
		object_init_ex(&config, tools_config_ce);
		ZEPHIR_CALL_METHOD(NULL, &config, "__construct", NULL, 7, toMerge);
		zephir_check_call_status();
	} else if (_0) {
		ZEPHIR_CPY_WRT(&config, toMerge);
	} else {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Invalid data type for merge.", "tools/Config.zep", 87);
		return;
	}
	...
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&result, this_ptr, "internalmerge", NULL, 8, &source, &target);
	zephir_check_call_status();
...
PHP_METHOD(Tools_Config, path) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&key, "array_shift", &_1, 9, &keys);
		ZEPHIR_UNREF(&keys);
		zephir_check_call_status();
...
PHP_METHOD(Tools_Config, toArray) {
	...
	array_init(&results);
	ZEPHIR_CALL_PARENT(&data, tools_config_ce, getThis(), "toarray", &_0, 0);
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "tools/Config.zep", 177);
...
PHP_METHOD(Tools_Config, internalMerge) {
	...
	zephir_is_iterable(&target, 0, "tools/Config.zep", 197);
	if (Z_TYPE_P(&target) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&target), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				zephir_array_fetch(&_6$$3, &source, &key, PH_NOISY, "tools/Config.zep", 188);
				_5$$3 = Z_TYPE_P(&_6$$3) == IS_ARRAY;
			}
			if (_5$$3) {
				zephir_array_fetch(&_8$$4, &source, &key, PH_NOISY | PH_READONLY, "tools/Config.zep", 189);
				ZEPHIR_CALL_METHOD(&_7$$4, this_ptr, "internalmerge", &_9, 8, &_8$$4, &value);
				zephir_check_call_status();
				zephir_array_update_zval(&source, &key, &_7$$4, PH_COPY | PH_SEPARATE);
			} else if (Z_TYPE_P(&key) == IS_LONG) {
				zephir_array_append(&source, &value, PH_SEPARATE, "tools/Config.zep", 191);
			} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$7) {
					...
					zephir_array_fetch(&_12$$7, &source, &key, PH_NOISY, "tools/Config.zep", 188);
					_11$$7 = Z_TYPE_P(&_12$$7) == IS_ARRAY;
				}
				if (_11$$7) {
					zephir_array_fetch(&_14$$8, &source, &key, PH_NOISY | PH_READONLY, "tools/Config.zep", 189);
					ZEPHIR_CALL_METHOD(&_13$$8, this_ptr, "internalmerge", &_9, 8, &_14$$8, &value);
					zephir_check_call_status();
...
PHP_METHOD(Tools_Config, setData) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		object_init_ex(&data, tools_config_ce);
		ZEPHIR_CALL_METHOD(NULL, &data, "__construct", NULL, 7, value);
		zephir_check_call_status();
	} else {
