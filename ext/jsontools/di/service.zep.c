...
ZEPHIR_INIT_CLASS(JsonTools_Di_Service) {
	...
	ZEPHIR_REGISTER_CLASS(JsonTools\\Di, Service, jsontools, di_service, jsontools_di_service_method_entry, 0);

	zend_declare_property_null(jsontools_di_service_ce, SL("definition"), ZEND_ACC_PROTECTED);

	/**
	 * @var bool
	 */
	zend_declare_property_bool(jsontools_di_service_ce, SL("resolved"), 0, ZEND_ACC_PROTECTED);

	/**
	 * @var bool
	 */
	zend_declare_property_bool(jsontools_di_service_ce, SL("shared"), 0, ZEND_ACC_PROTECTED);

	zend_declare_property_null(jsontools_di_service_ce, SL("sharedInstance"), ZEND_ACC_PROTECTED);

	zend_class_implements(jsontools_di_service_ce, 1, jsontools_di_serviceinterface_ce);
	return SUCCESS;
...
PHP_METHOD(JsonTools_Di_Service, getParameter) {
	...
	if (UNEXPECTED(Z_TYPE_P(&definition) != IS_ARRAY)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(jsontools_di_exception_ce, "Definition must be an array to obtain its parameters", "jsontools/Di/Service.zep", 76);
		return;
	}
...
PHP_METHOD(JsonTools_Di_Service, resolve) {
	...
	if (Z_TYPE_P(&definition) == IS_STRING) {
		...
		if (Z_TYPE_P(&definition) == IS_OBJECT) {
			...
			if (Z_TYPE_P(&definition) == IS_ARRAY) {
				...
				object_init_ex(&builder, jsontools_di_service_builder_ce);
				if (zephir_has_constructor(&builder)) {
					ZEPHIR_CALL_METHOD(NULL, &builder, "__construct", NULL, 0);
					zephir_check_call_status();
				}
				ZEPHIR_CALL_METHOD(&instance, &builder, "build", NULL, 19, container, &definition, parameters);
				zephir_check_call_status();
			} else {
				found = 0;
			}
	...
	if (UNEXPECTED(found == 0)) {
		...
		object_init_ex(&_5$$21, jsontools_di_exception_serviceresolutionexception_ce);
		ZEPHIR_CALL_METHOD(NULL, &_5$$21, "__construct", NULL, 12);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_5$$21, "jsontools/Di/Service.zep", 199);
		ZEPHIR_MM_RESTORE();
		return;
	}
	if (zephir_is_true(&shared)) {
...
PHP_METHOD(JsonTools_Di_Service, setParameter) {
	...
	ZEPHIR_CPY_WRT(&definition, &_0);
	if (UNEXPECTED(Z_TYPE_P(&definition) != IS_ARRAY)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(jsontools_di_exception_ce, "Definition must be an array to update its parameters", "jsontools/Di/Service.zep", 234);
		return;
	}
