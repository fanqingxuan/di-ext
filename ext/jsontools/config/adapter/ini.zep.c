 ...
 *
 * ```php
 * $config = new \JsonTools\Config\Adapter\Ini(
 *     "path/config-with-constants.ini",
 *     INI_SCANNER_NORMAL
 * );
 * ```
 */
ZEPHIR_INIT_CLASS(JsonTools_Config_Adapter_Ini) {

	ZEPHIR_REGISTER_CLASS_EX(JsonTools\\Config\\Adapter, Ini, jsontools, config_adapter_ini, jsontools_config_ce, jsontools_config_adapter_ini_method_entry, 0);
 ...
 * Ini constructor.
 */
PHP_METHOD(JsonTools_Config_Adapter_Ini, __construct) {
	...
	ZEPHIR_CALL_FUNCTION(&iniConfig, "parse_ini_file", NULL, 11, &filePath, &__$true, mode);
	...
	if (UNEXPECTED(ZEPHIR_IS_FALSE_IDENTICAL(&iniConfig))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_0$$4, "__construct", NULL, 12, &_2$$4);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_0$$4, "jsontools/Config/Adapter/Ini.zep", 68);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_INIT_VAR(&config);
	array_init(&config);
	zephir_is_iterable(&iniConfig, 0, "jsontools/Config/Adapter/Ini.zep", 95);
	if (Z_TYPE_P(&iniConfig) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&iniConfig), _5, _6, _3)
		{
			...
			if (Z_TYPE_P(&directives) == IS_ARRAY) {
				...
				array_init(&sections);
				zephir_is_iterable(&directives, 0, "jsontools/Config/Adapter/Ini.zep", 84);
				if (Z_TYPE_P(&directives) == IS_ARRAY) {
					ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&directives), _9$$6, _10$$6, _7$$6)
					{
						...
						zephir_check_call_status();
						zephir_array_append(&sections, &_11$$7, PH_SEPARATE, "jsontools/Config/Adapter/Ini.zep", 81);
					...
					while (1) {
						...
						zephir_check_call_status();
							...
							zephir_check_call_status();
							zephir_array_append(&sections, &_14$$8, PH_SEPARATE, "jsontools/Config/Adapter/Ini.zep", 81);
				...
				if (Z_TYPE_P(&directives) == IS_ARRAY) {
					...
					if (Z_TYPE_P(&directives) == IS_ARRAY) {
						ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&directives), _22$$12, _23$$12, _20$$12)
						{
							...
							zephir_check_call_status();
							zephir_array_append(&sections, &_24$$13, PH_SEPARATE, "jsontools/Config/Adapter/Ini.zep", 81);
						} ZEND_HASH_FOREACH_END();
					} else {
						ZEPHIR_CALL_METHOD(NULL, &directives, "rewind", NULL, 0);
						zephir_check_call_status();
						while (1) {
							...
							zephir_check_call_status();
								...
								zephir_check_call_status();
								zephir_array_append(&sections, &_26$$14, PH_SEPARATE, "jsontools/Config/Adapter/Ini.zep", 81);
							ZEPHIR_CALL_METHOD(NULL, &directives, "next", NULL, 0);
							zephir_check_call_status();
						}
					}
	...
	ZEPHIR_INIT_NVAR(&directives);
	ZEPHIR_INIT_NVAR(&section);
	ZEPHIR_CALL_PARENT(NULL, jsontools_config_adapter_ini_ce, getThis(), "__construct", &_31, 0, &config);
	zephir_check_call_status();
	ZEPHIR_MM_RESTORE();
 ...
 * implementation.
 */
PHP_METHOD(JsonTools_Config_Adapter_Ini, cast) {
	...
	if (Z_TYPE_P(ini) == IS_ARRAY) {
		zephir_is_iterable(ini, 1, "jsontools/Config/Adapter/Ini.zep", 111);
		if (Z_TYPE_P(ini) == IS_ARRAY) {
			ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(ini), _2$$3, _3$$3, _0$$3)
			{
				...
				ZEPHIR_CALL_METHOD(&_4$$4, this_ptr, "cast", &_5, 13, &value);
				zephir_check_call_status();
			...
			while (1) {
				...
				zephir_check_call_status();
					ZEPHIR_CALL_METHOD(&_6$$5, this_ptr, "cast", &_5, 13, &value);
					zephir_check_call_status();
 ...
 */
PHP_METHOD(JsonTools_Config_Adapter_Ini, parseIniString) {
	...
	zephir_create_array(return_value, 1, 0);
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "parseinistring", NULL, 14, &path, value);
	zephir_check_call_status();
