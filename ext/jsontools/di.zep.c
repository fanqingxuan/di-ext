...
/**
 * JsonTools\Di is a component that implements Dependency Injection/Service
 * Location of services and it's itself a container for them.
 *
 * Since JsonTools is highly decoupled, JsonTools\Di is essential to integrate the
...
ZEPHIR_INIT_CLASS(JsonTools_Di) {
	...
	ZEPHIR_REGISTER_CLASS(JsonTools, Di, jsontools, di, jsontools_di_method_entry, 0);

	/**
	 * List of registered services
	 */
	zend_declare_property_null(jsontools_di_ce, SL("services"), ZEND_ACC_PROTECTED);

	/**
	 * List of shared instances
	 */
	zend_declare_property_null(jsontools_di_ce, SL("sharedInstances"), ZEND_ACC_PROTECTED);

	/**
	 * Latest DI build
	 */
	zend_declare_property_null(jsontools_di_ce, SL("_default"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);

	zend_class_implements(jsontools_di_ce, 1, jsontools_di_diinterface_ce);
	return SUCCESS;
...
PHP_METHOD(JsonTools_Di, __construct) {
	...
	zephir_read_static_property_ce(&_0, jsontools_di_ce, SL("_default"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_is_true(&_0))) {
		zephir_update_static_property_ce(jsontools_di_ce, ZEND_STRL("_default"), this_ptr);
	}
...
PHP_METHOD(JsonTools_Di, __call) {
	...
	if (zephir_start_with_str(&method, SL("get"))) {
		...
		ZEPHIR_CALL_FUNCTION(&possibleService, "lcfirst", &_2, 15, &_1$$3);
	...
	if (zephir_start_with_str(&method, SL("set"))) {
		...
		if (zephir_array_isset_long_fetch(&definition, &arguments, 0, 0)) {
			...
			ZEPHIR_CALL_FUNCTION(&_6$$6, "lcfirst", &_2, 15, &_5$$6);
			zephir_check_call_status();
...
PHP_METHOD(JsonTools_Di, attempt) {
	...
	ZEPHIR_INIT_VAR(&_1);
	object_init_ex(&_1, jsontools_di_service_ce);
	if (shared) {
		ZVAL_BOOL(&_2, 1);
	} else {
		ZVAL_BOOL(&_2, 0);
	}
	ZEPHIR_CALL_METHOD(NULL, &_1, "__construct", NULL, 16, definition, &_2);
	zephir_check_call_status();
	...
	zephir_array_fetch(&_4, &_3, &name, PH_NOISY | PH_READONLY, "jsontools/Di.zep", 136);
	RETURN_CTOR(&_4);
...
PHP_METHOD(JsonTools_Di, get) {
	...
	if (zephir_array_isset_fetch(&service, &_0, &name, 0)) {
		...
		if (_1$$3) {
			...
			zephir_array_fetch(&_4$$4, &_3$$4, &name, PH_NOISY | PH_READONLY, "jsontools/Di.zep", 154);
			RETURN_CTOR(&_4$$4);
		}
	}
	if (Z_TYPE_P(&instance) != IS_OBJECT) {
		if (Z_TYPE_P(&service) != IS_NULL) {
			...
			if (EG(exception)) {
				...
				if (zephir_instance_of_ev(&_5$$6, jsontools_di_exception_serviceresolutionexception_ce)) {
					zend_clear_exception(TSRMLS_C);
					ZEPHIR_CPY_WRT(&_6$$6, &_5$$6);
					ZEPHIR_INIT_VAR(&_7$$8);
					object_init_ex(&_7$$8, jsontools_di_exception_ce);
					ZEPHIR_INIT_VAR(&_8$$8);
					ZEPHIR_CONCAT_SVS(&_8$$8, "Service '", &name, "' cannot be resolved");
					ZEPHIR_CALL_METHOD(NULL, &_7$$8, "__construct", NULL, 12, &_8$$8);
					zephir_check_call_status();
					zephir_throw_exception_debug(&_7$$8, "jsontools/Di.zep", 167);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
			...
			if (UNEXPECTED(!(zephir_class_exists(&name, 1)))) {
				...
				object_init_ex(&_9$$11, jsontools_di_exception_ce);
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_9$$11, "jsontools/Di.zep", 182);
				ZEPHIR_MM_RESTORE();
				return;
			}
	...
	if (Z_TYPE_P(&instance) == IS_OBJECT) {
		if (zephir_instance_of_ev(&instance, jsontools_di_injectionawareinterface_ce)) {
...
PHP_METHOD(JsonTools_Di, getRaw) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		ZEPHIR_INIT_VAR(&_1$$3);
		object_init_ex(&_1$$3, jsontools_di_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "jsontools/Di.zep", 230);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(JsonTools_Di, getService) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 12, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "jsontools/Di.zep", 246);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(JsonTools_Di, loadFromConfig) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&services, 0, "jsontools/Di.zep", 296);
	if (Z_TYPE_P(&services) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&services), _2, _3, _0)
		{
			...
			if (_4$$3) {
				...
				zephir_array_fetch_string(&_5$$3, &service, SL("shared"), PH_NOISY, "jsontools/Di.zep", 294);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_8$$4) {
					...
					zephir_array_fetch_string(&_9$$4, &service, SL("shared"), PH_NOISY, "jsontools/Di.zep", 294);
...
PHP_METHOD(JsonTools_Di, loadFromPhp) {
	...
	object_init_ex(&services, jsontools_config_adapter_php_ce);
	ZEPHIR_CALL_METHOD(NULL, &services, "__construct", NULL, 17, &filePath);
	zephir_check_call_status();
 ...
 *
 *```php
 * $di["request"] = new \JsonTools\Http\Request();
 *```
 */
...
PHP_METHOD(JsonTools_Di, reset) {
	...
	zephir_update_static_property_ce(jsontools_di_ce, ZEND_STRL("_default"), &__$null);
...
PHP_METHOD(JsonTools_Di, set) {
	...
	ZEPHIR_INIT_VAR(&_0);
	object_init_ex(&_0, jsontools_di_service_ce);
	if (shared) {
		ZVAL_BOOL(&_1, 1);
	} else {
		ZVAL_BOOL(&_1, 0);
	}
	ZEPHIR_CALL_METHOD(NULL, &_0, "__construct", NULL, 16, definition, &_1);
	zephir_check_call_status();
	...
	zephir_array_fetch(&_3, &_2, &name, PH_NOISY | PH_READONLY, "jsontools/Di.zep", 445);
	RETURN_CTOR(&_3);
...
PHP_METHOD(JsonTools_Di, __set) {
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "setshared", NULL, 0, &name, definition);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_METHOD(JsonTools_Di, setDefault) {
	...
	zephir_update_static_property_ce(jsontools_di_ce, ZEND_STRL("_default"), container);
