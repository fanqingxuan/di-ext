...
/**
 * This file is part of the JsonDi.
 *
 * (c) JsonDi Team <team@datas.com>
 *
...
ZEPHIR_INIT_CLASS(JsonDi_Helper_Json) {
	...
	ZEPHIR_REGISTER_CLASS(JsonDi\\Helper, Json, jsondi, helper_json, jsondi_helper_json_method_entry, 0);
...
PHP_METHOD(JsonDi_Helper_Json, decode) {
	...
	if (UNEXPECTED(!ZEPHIR_IS_LONG_IDENTICAL(&_3, 0))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_4$$3, "jsondi/Helper/Json.zep", 60);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(JsonDi_Helper_Json, encode) {
	...
	if (UNEXPECTED(!ZEPHIR_IS_LONG_IDENTICAL(&_2, 0))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_3$$3, "jsondi/Helper/Json.zep", 104);
		ZEPHIR_MM_RESTORE();
		return;
	}
