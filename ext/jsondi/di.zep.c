...
/**
 * JsonDi\Di is a component that implements Dependency Injection/Service
 * Location of services and it's itself a container for them.
 *
 * Since JsonDi is highly decoupled, JsonDi\Di is essential to integrate the
...
ZEPHIR_INIT_CLASS(JsonDi_Di) {
	...
	ZEPHIR_REGISTER_CLASS(JsonDi, Di, jsondi, di, jsondi_di_method_entry, 0);

	/**
	 * List of registered services
	 */
	zend_declare_property_null(jsondi_di_ce, SL("services"), ZEND_ACC_PROTECTED);

	/**
	 * List of shared instances
	 */
	zend_declare_property_null(jsondi_di_ce, SL("sharedInstances"), ZEND_ACC_PROTECTED);

	/**
	 * Latest DI build
	 */
	zend_declare_property_null(jsondi_di_ce, SL("_default"), ZEND_ACC_PROTECTED|ZEND_ACC_STATIC);

	zend_class_implements(jsondi_di_ce, 1, jsondi_di_diinterface_ce);
	return SUCCESS;
...
PHP_METHOD(JsonDi_Di, __construct) {
	...
	zephir_read_static_property_ce(&_0, jsondi_di_ce, SL("_default"), PH_NOISY_CC | PH_READONLY);
	if (!(zephir_is_true(&_0))) {
		zephir_update_static_property_ce(jsondi_di_ce, ZEND_STRL("_default"), this_ptr);
	}
...
PHP_METHOD(JsonDi_Di, attempt) {
	...
	ZEPHIR_INIT_VAR(&_1);
	object_init_ex(&_1, jsondi_di_service_ce);
	if (shared) {
		ZVAL_BOOL(&_2, 1);
	} else {
		ZVAL_BOOL(&_2, 0);
	...
	zephir_array_fetch(&_4, &_3, &name, PH_NOISY | PH_READONLY, "jsondi/Di.zep", 136);
	RETURN_CTOR(&_4);
...
PHP_METHOD(JsonDi_Di, get) {
	...
	if (zephir_array_isset_fetch(&service, &_0, &name, 0)) {
		...
		if (_1$$3) {
			...
			zephir_array_fetch(&_4$$4, &_3$$4, &name, PH_NOISY | PH_READONLY, "jsondi/Di.zep", 154);
			RETURN_CTOR(&_4$$4);
		}
	}
	if (Z_TYPE_P(&instance) != IS_OBJECT) {
		if (Z_TYPE_P(&service) != IS_NULL) {
			...
			if (EG(exception)) {
				...
				if (zephir_instance_of_ev(&_5$$6, jsondi_di_exception_serviceresolutionexception_ce)) {
					zend_clear_exception(TSRMLS_C);
					ZEPHIR_CPY_WRT(&_6$$6, &_5$$6);
					ZEPHIR_INIT_VAR(&_7$$8);
					object_init_ex(&_7$$8, jsondi_di_exception_ce);
					ZEPHIR_INIT_VAR(&_8$$8);
					ZEPHIR_CONCAT_SVS(&_8$$8, "Service '", &name, "' cannot be resolved");
					ZEPHIR_CALL_METHOD(NULL, &_7$$8, "__construct", NULL, 12, &_8$$8);
					zephir_check_call_status();
					zephir_throw_exception_debug(&_7$$8, "jsondi/Di.zep", 167);
					ZEPHIR_MM_RESTORE();
					return;
				}
			}
			...
			if (UNEXPECTED(!(zephir_class_exists(&name, 1)))) {
				...
				object_init_ex(&_9$$11, jsondi_di_exception_ce);
				...
				zephir_check_call_status();
				zephir_throw_exception_debug(&_9$$11, "jsondi/Di.zep", 182);
				ZEPHIR_MM_RESTORE();
				return;
			}
	...
	if (Z_TYPE_P(&instance) == IS_OBJECT) {
		if (zephir_instance_of_ev(&instance, jsondi_di_injectionawareinterface_ce)) {
...
PHP_METHOD(JsonDi_Di, getRaw) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		ZEPHIR_INIT_VAR(&_1$$3);
		object_init_ex(&_1$$3, jsondi_di_exception_ce);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "jsondi/Di.zep", 230);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(JsonDi_Di, getService) {
	...
	if (UNEXPECTED(!(zephir_array_isset_fetch(&service, &_0, &name, 0)))) {
		...
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 12, &_2$$3);
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$3, "jsondi/Di.zep", 246);
		ZEPHIR_MM_RESTORE();
		return;
	}
...
PHP_METHOD(JsonDi_Di, loadFromConfig) {
	...
	zephir_check_call_status();
	zephir_is_iterable(&services, 0, "jsondi/Di.zep", 296);
	if (Z_TYPE_P(&services) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&services), _2, _3, _0)
		{
			...
			if (_4$$3) {
				...
				zephir_array_fetch_string(&_5$$3, &service, SL("shared"), PH_NOISY, "jsondi/Di.zep", 294);
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_8$$4) {
					...
					zephir_array_fetch_string(&_9$$4, &service, SL("shared"), PH_NOISY, "jsondi/Di.zep", 294);
...
PHP_METHOD(JsonDi_Di, loadFromPhp) {
	...
	object_init_ex(&services, jsondi_config_adapter_php_ce);
 ...
 *
 *```php
 * $di["request"] = new \JsonDi\Http\Request();
 *```
 */
...
PHP_METHOD(JsonDi_Di, reset) {
	...
	zephir_update_static_property_ce(jsondi_di_ce, ZEND_STRL("_default"), &__$null);
...
PHP_METHOD(JsonDi_Di, set) {
	...
	ZEPHIR_INIT_VAR(&_0);
	object_init_ex(&_0, jsondi_di_service_ce);
	if (shared) {
		ZVAL_BOOL(&_1, 1);
	} else {
		ZVAL_BOOL(&_1, 0);
	...
	zephir_array_fetch(&_3, &_2, &name, PH_NOISY | PH_READONLY, "jsondi/Di.zep", 445);
	RETURN_CTOR(&_3);
...
PHP_METHOD(JsonDi_Di, setDefault) {
	...
	zephir_update_static_property_ce(jsondi_di_ce, ZEND_STRL("_default"), container);
