 ...
 */
/**
 * `JsonDi\Collection` is a supercharged object oriented array. It implements:
...
ZEPHIR_INIT_CLASS(JsonDi_Collection) {
	...
	ZEPHIR_REGISTER_CLASS(JsonDi, Collection, jsondi, collection, jsondi_collection_method_entry, 0);

	/**
	 * @var array
	 */
	zend_declare_property_null(jsondi_collection_ce, SL("data"), ZEND_ACC_PROTECTED);

	/**
	 * @var bool
	 */
	zend_declare_property_bool(jsondi_collection_ce, SL("insensitive"), 1, ZEND_ACC_PROTECTED);

	/**
	 * @var array
	 */
	zend_declare_property_null(jsondi_collection_ce, SL("lowerKeys"), ZEND_ACC_PROTECTED);

	jsondi_collection_ce->create_object = zephir_init_properties_JsonDi_Collection;

	zend_class_implements(jsondi_collection_ce, 1, zend_ce_arrayaccess);
	zend_class_implements(jsondi_collection_ce, 1, spl_ce_Countable);
	zend_class_implements(jsondi_collection_ce, 1, zend_ce_aggregate);
	zend_class_implements(jsondi_collection_ce, 1, zephir_get_internal_ce(SL("jsonserializable")));
	zend_class_implements(jsondi_collection_ce, 1, zend_ce_serializable);
	return SUCCESS;
...
PHP_METHOD(JsonDi_Collection, get) {
	...
	zephir_array_fetch(&value, &_3, &key, PH_NOISY, "jsondi/Collection.zep", 133);
...
PHP_METHOD(JsonDi_Collection, init) {
	...
	zephir_is_iterable(&data, 0, "jsondi/Collection.zep", 187);
...
PHP_METHOD(JsonDi_Collection, jsonSerialize) {
	...
	array_init(&records);
	zephir_read_property(&_0, this_ptr, SL("data"), PH_NOISY_CC | PH_READONLY);
	zephir_is_iterable(&_0, 0, "jsondi/Collection.zep", 208);
...
PHP_METHOD(JsonDi_Collection, remove) {
	...
	if (UNEXPECTED(zephir_is_true(&_0))) {
		...
		zephir_array_fetch(&key, &lowerKeys, &element, PH_NOISY | PH_READONLY, "jsondi/Collection.zep", 270);
...
PHP_METHOD(JsonDi_Collection, toJson) {
	...
	ZEPHIR_RETURN_CALL_CE_STATIC(jsondi_helper_json_ce, "encode", &_0, 5, &_1, &_2);
	zephir_check_call_status();
	RETURN_MM();
