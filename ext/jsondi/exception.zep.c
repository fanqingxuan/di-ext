...
ZEPHIR_INIT_CLASS(JsonDi_Exception) {
	...
	ZEPHIR_REGISTER_CLASS_EX(JsonDi, Exception, jsondi, exception, zend_exception_get_default(TSRMLS_C), jsondi_exception_method_entry, 0);

	zend_class_implements(jsondi_exception_ce, 1, zephir_get_internal_ce(SL("throwable")));
	return SUCCESS;
