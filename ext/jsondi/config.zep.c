...
ZEPHIR_INIT_CLASS(JsonDi_Config) {
	...
	ZEPHIR_REGISTER_CLASS_EX(JsonDi, Config, jsondi, config, jsondi_collection_ce, jsondi_config_method_entry, 0);

	/**
	 * @var string
	 */
	zend_declare_property_null(jsondi_config_ce, SL("pathDelimiter"), ZEND_ACC_PROTECTED);

	zephir_declare_class_constant_string(jsondi_config_ce, SL("DEFAULT_PATH_DELIMITER"), ".");
...
PHP_METHOD(JsonDi_Config, merge) {
	...
	if (_0) {
		_0 = zephir_instance_of_ev(toMerge, jsondi_config_ce);
	}
	if (Z_TYPE_P(toMerge) == IS_ARRAY) {
		ZEPHIR_INIT_VAR(&config);
		object_init_ex(&config, jsondi_config_ce);
		ZEPHIR_CALL_METHOD(NULL, &config, "__construct", NULL, 7, toMerge);
		zephir_check_call_status();
	} else if (_0) {
		ZEPHIR_CPY_WRT(&config, toMerge);
	} else {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(zend_exception_get_default(TSRMLS_C), "Invalid data type for merge.", "jsondi/Config.zep", 87);
		return;
	}
...
PHP_METHOD(JsonDi_Config, toArray) {
	...
	array_init(&results);
	ZEPHIR_CALL_PARENT(&data, jsondi_config_ce, getThis(), "toarray", &_0, 0);
	zephir_check_call_status();
	zephir_is_iterable(&data, 0, "jsondi/Config.zep", 177);
...
PHP_METHOD(JsonDi_Config, internalMerge) {
	...
	zephir_is_iterable(&target, 0, "jsondi/Config.zep", 197);
	if (Z_TYPE_P(&target) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&target), _2, _3, _0)
		{
			...
			if (_5$$3) {
				...
				zephir_array_fetch(&_6$$3, &source, &key, PH_NOISY, "jsondi/Config.zep", 188);
				_5$$3 = Z_TYPE_P(&_6$$3) == IS_ARRAY;
			}
			if (_5$$3) {
				zephir_array_fetch(&_8$$4, &source, &key, PH_NOISY | PH_READONLY, "jsondi/Config.zep", 189);
				...
				zephir_array_append(&source, &value, PH_SEPARATE, "jsondi/Config.zep", 191);
			} else {
		...
		while (1) {
			...
			zephir_check_call_status();
				...
				if (_11$$7) {
					...
					zephir_array_fetch(&_12$$7, &source, &key, PH_NOISY, "jsondi/Config.zep", 188);
					_11$$7 = Z_TYPE_P(&_12$$7) == IS_ARRAY;
				}
				if (_11$$7) {
					zephir_array_fetch(&_14$$8, &source, &key, PH_NOISY | PH_READONLY, "jsondi/Config.zep", 189);
...
PHP_METHOD(JsonDi_Config, setData) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		object_init_ex(&data, jsondi_config_ce);
